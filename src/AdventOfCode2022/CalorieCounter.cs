    ...
    internal class CalorieCounter : IChallenge
    {
        ...
        public object SolvePart1(string input)
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    throw new Exception($"Line value '{line}' is not a valid input for the snack list. Input should be a integer of calories");
                }
        ...
        {
            public static IComparer<SnackCount> Default = Comparer<SnackCount>.Create((left, right) => -left.Calories.CompareTo(right.Calories));

            public override string ToString()
            {
                return $"Elf #{Elf}, Calories = {Calories}";
            }
        }
    }
}
